...
//
// 使用前提:
//   把本目录下所有CSV放到 Neo4j 的 import/mechanical_fracture/ 文件夹
//
// 关系体系:
...
// -------- 2. 加载节点 --------
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/nodes_failure_mode.csv' AS row
CREATE (:FailureMode {entityId: row.`entityId:ID`, name: row.name, entityType: row.entityType, detectTime: row.detectTime});

LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/nodes_mechanism.csv' AS row
CREATE (:Mechanism {mechId: row.`mechId:ID`, name: row.name, description: row.description});

LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/nodes_design_param.csv' AS row
...
// -------- 3. HAS_SUBTYPE (根→子失效) --------
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_subtype.csv' AS row
MATCH (a:FailureMode {entityId: row.`:START_ID`})
MATCH (b:FailureMode {entityId: row.`:END_ID`})
CREATE (a)-[:HAS_SUBTYPE]->(b);

// -------- 4. CAUSES (机理→失效, 主因) --------
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_causes.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_contributes.csv' AS row
MATCH (m:Mechanism {mechId: row.`:START_ID`})
MATCH (f:FailureMode {entityId: row.`:END_ID`})
CREATE (m)-[:CONTRIBUTES {description: row.description}]->(f);

// -------- 6. INFLUENCES (参数→机理) --------
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_influences.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_directly_affects.csv' AS row
MATCH (p:DesignParam {entityId: row.`:START_ID`})
MATCH (f:FailureMode {entityId: row.`:END_ID`})
...
// 8a. 机理→机理 EVOLVES_TO
LOAD CSV WITH HEADERS FROM 'file:///mechanical_fracture/rels_evolves_to.csv' AS row
...
// 从根展开:    MATCH path=(:FailureMode {entityId:'F_FRAC'})-[*1..3]-() RETURN path
